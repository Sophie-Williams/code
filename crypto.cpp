...
Crypto::hash::hash(Algorithms algorithm, const CryptContext & context, HCRYPTKEY hKey) : context(context)
{
	...
	switch (algorithm) {
	case sha256:
		if (!CryptCreateHash(this->context.handle(), CALG_SHA_256, 0, 0, &hHash))
			throw std::exception("CryptCreateHash() failed.");
		break;
	case sha384:
		if (!CryptCreateHash(this->context.handle(), CALG_SHA_384, 0, 0, &hHash))
			throw std::exception("CryptCreateHash() failed.");
		break;
	case sha512:
		if (!CryptCreateHash(this->context.handle(), CALG_SHA_512, 0, 0, &hHash))
			throw std::exception("CryptCreateHash() failed.");
		break;
	case md5:
		if (!CryptCreateHash(this->context.handle(), CALG_MD5, 0, 0, &hHash))
			throw std::exception("CryptCreateHash() failed.");
		break;
	case hmac:
		if (!CryptCreateHash(this->context.handle(), CALG_MD5, hKey, 0, &hHash))
			throw std::exception("CryptCreateHash() failed.");
		break;
	}
}
...
bool Crypto::hash::hash_data(LPCVOID data, DWORD dwDataLen)
{
	if (!available())
		return false;
	return ::CryptHashData(hHash, (BYTE*)data, dwDataLen, NULL) == TRUE;
}
